...
//builds a 3x9 block of the final matrix writes the hchunk, and takes in dchunk
void homographyBlock(mrange &h, mrow &d)
{
  assert(h.size1()==3 && h.size2()==9);
  float x = d(0), y=d(1), xp=d(2), yp=d(3);

  h(0,0)= 0;       h(0,1)=0;        h(0,2)=0;
  h(0,3)=-x;       h(0,4)=-y;       h(0,5)=-1;
  h(0,6)=yp*x;     h(0,7)=yp*y;     h(0,8)=yp;

  h(1,0)=x;         h(1,1)=y;       h(1,2)=1;
  h(1,3)=0;         h(1,4)=0;       h(1,5)=0;
  h(1,6)=-xp*x;     h(1,7)=-xp*y;   h(1,8)=-xp;

  h(2,0)=-yp*x;     h(2,1)=-yp*y;   h(2,2)=-yp;
  h(2,3)=xp*x;      h(2,4)=xp*y;    h(2,5)=xp;
  h(2,6)=0;         h(2,7)=0;       h(2,8)=0;
}

//x' F x ==0 //x - img1 xp -img2
...
void buildLeastSqHomographyMatrix(mat &data, mat &h)
{
  int height = data.size1()*3;
  h.resize(height, 9);
...
bool solveLeastSqHomography(mat &data, mat &h)
{
  mat dataS;//scaled data
  mat H;//big least Sq matrix
  vec s1,s2;
  conditionData(data, dataS, s1, s2);
  buildLeastSqHomographyMatrix(dataS,H);
  const int m = H.size1();
  const int n = H.size2();
  ...
  assert(s(7)> 1e-3);//need at least 3 non 0 singular values
  ...
  mat t1(3,3), t2(3,3);
   ...
   t2i.assign(ublas::identity_matrix<real_t>(A.size1()));
   lu_substitute(A,pm, t2i);
   //////
...
int computeNoInliersHomography(mat &data, mat &f, ublas::vector<bool> &inlier_mask, double &dist_std)
{
  double T_DIST = 0.5;//dist>0.1 pixels = outlier might want to make it sqrt(5.99)*sigma
  int num_inlier = 0;
  double m_dist=0;
  ...
  for (int i=0;i<data.size1();i++)
    {
      ...
      x1(0)=data(i,0);x1(1)=data(i,1);x1(2)=1;
      x2(0)=data(i,2);x2(1)=data(i,3);x2(2)=1;
      vec err=prod (f, x1);//H x1
      err(0)/=err(2);
      err(1)/=err(2);
      err(2)/=err(2);
      dist(i)=norm_2(err-x2);
      if (dist(i)<T_DIST)
	{
	  num_inlier++;
	  inlier_mask(i)=true;
...
void ransacHomography(mat &data, mat &h)
{
  int N = 1000, s = 5, sample_cnt = 0;
  const double p = 0.99;
  double best_std; 
  ...
  while(N > sample_cnt)
  {
      ...
      for (int i=0;i<s;i++)
	{
      ...
      bool solved = solveLeastSqHomography(dataSel, curF);
      double cur_std;
      if (solved)
	{
	  int cur_ninliers=
	    computeNoInliersHomography(data, curF, cur_inlier_mask, cur_std);
   ...
   if (best_ninliers>4)
   {
     ...
     for (int i=0;i<data.size1();i++)
      ...
      bool solved = solveLeastSqHomography(dataSel, h);
      printf("Took %d iterations, std=%f\n", sample_cnt, best_std);
